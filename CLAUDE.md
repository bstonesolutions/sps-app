...
- `QUO_WEBHOOK_KEY` — **inbound SMS funnel** (`api/sms-intake.js`). A random string YOU set, placed in the Quo webhook URL as `?key=<this>` — the endpoint's primary auth (Quo POSTs the URL verbatim). Optional `QUO_WEBHOOK_SECRET` = the Quo/OpenPhone webhook **signing secret** (base64, from the webhook's "Reveal signing secret" or the create-webhook API `key` field) → when set, the endpoint ALSO verifies the `openphone-signature` HMAC. Ships dark until `QUO_WEBHOOK_KEY` is set. Someone texting the business number → `message.received` webhook → the text lands in `sps_inbox` (channel 'sms', from_phone) alongside email: a known client's text is tagged "Client", an unknown number is AI-triaged (lead/bill/other) and lead-auto-imported like an email lead. Outbound texts we send ride the same stream with `direction:"outgoing"` and are dropped. Setup: **Quo → Settings → Webhooks → create** for event `message.received`, URL `https://spsway.app/api/sms-intake?key=<QUO_WEBHOOK_KEY>` (or via API `POST api.openphone.com/v1/webhooks/messages` with `Authorization: <QUO_API_KEY>`). Requires the sps_inbox channel/from_phone columns (SQL below).
